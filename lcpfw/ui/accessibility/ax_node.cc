...
base::string16 AXNode::GetHypertext() const {
  ...
  DCHECK_EQ(int{ (int)embedded_character_str.length()}, kEmbeddedCharacterLength);
...
int AXNode::GetInnerTextLength() const {
  ...
  if (IsLeaf() && !is_plain_text_field_with_descendants)
    return int{(int)GetInnerText().length()};
...
base::Optional<int> AXNode::GetTableColCount() const {
  ...
  if (!table_info)
    return base::nullopt;
  return int{(int)table_info->col_count};
}
...
base::Optional<int> AXNode::GetTableRowCount() const {
  ...
  if (!table_info)
    return base::nullopt;
  return int{(int)table_info->row_count};
}
...
AXNode* AXNode::GetTableCellFromIndex(int index) const {
  ...
  if (index < 0 || size_t{(size_t)index} >= table_info->unique_cell_ids.size()) {
    return nullptr;
  }

  return tree_->GetFromId(table_info->unique_cell_ids[size_t{(size_t)index}]);
}
...
AXNode* AXNode::GetTableCellFromCoords(int row_index, int col_index) const {
  ...
  if (row_index < 0 || size_t{(size_t)row_index} >= table_info->row_count ||
      col_index < 0 || size_t{(size_t)col_index} >= table_info->col_count) {
    return nullptr;
  }

  return tree_->GetFromId(
      table_info->cell_ids[size_t{(size_t)row_index}][size_t{(size_t)col_index}]);
}
...
std::vector<AXNodeID> AXNode::GetTableColHeaderNodeIds(int col_index) const {
  ...
  if (col_index < 0 || size_t{(size_t)col_index} >= table_info->col_count)
    return std::vector<AXNodeID>();

  return std::vector<AXNodeID>(table_info->col_headers[size_t{(size_t)col_index}]);
}
...
std::vector<AXNodeID> AXNode::GetTableRowHeaderNodeIds(int row_index) const {
  ...
  return std::vector<AXNodeID>(table_info->row_headers[size_t{(size_t)row_index}]);
}
...
base::Optional<int> AXNode::GetTableCellColIndex() const {
  ...
  return int{(int)table_info->cell_data_vector[*index].col_index};
}
...
base::Optional<int> AXNode::GetTableCellRowIndex() const {
  ...
  return int{(int)table_info->cell_data_vector[*index].row_index};
}
...
base::Optional<int> AXNode::GetTableCellAriaColIndex() const {
  ...
  return int{(int)table_info->cell_data_vector[*index].aria_col_index};
}
...
base::Optional<int> AXNode::GetTableCellAriaRowIndex() const {
  ...
  return int{(int)table_info->cell_data_vector[*index].aria_row_index};
}
