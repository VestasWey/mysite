...
namespace mctm
{
    ...
    bool IPCChannel::ProcessOutgoingMessages()
    {
        ...
        case mctm::IPCChannel::MODE_SERVER:
            if (client_key_ != 0)
            {
                ...
                ret = pipe_srv_->Send(client_key_, (const char*)msg->data(), (unsigned long)current_send_bytes);
            }
            break;
        case mctm::IPCChannel::MODE_CLIENT:
            {
                ...
                size_t surplus = msg->size() - msg->offset();
                current_send_bytes = std::min(surplus, kIOBufferSize);
                ret = pipe_clt_->Send((const char*)msg->data(), (unsigned long)current_send_bytes);
            }
            break;
        default:
            break;
    ...
    std::unique_ptr<IPCMessage> IPCChannel::ProcessIncomingMessages(const char* data, unsigned int len)
    {
        ...
        if (msg_pickle)
        {
            int lent = static_cast<int>(msg_pickle - input_overflow_buf_.data());
            DCHECK(lent > 0);
            msg = std::make_unique<IPCMessage>(input_overflow_buf_.data(), lent);
        }
        return msg;
    }
