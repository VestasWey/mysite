...
class PostTaskAndReplyWithTraitsTaskRunner
 ...
 private:
  bool PostTask(const Location& from_here, OnceClosure task) override {
    ThreadPool::PostTask(from_here, traits_.value(), std::move(task));
    return true;
  }

  //const TaskTraits traits_;
  Optional<TaskTraits> traits_;
};
...
bool ThreadPool::PostDelayedTask(const Location& from_here,
                                 OnceClosure task,
                                 TimeDelta delay) {
  return ThreadPool::PostDelayedTask(from_here, { base::MayBlock() }, std::move(task), delay);
}
...
bool ThreadPool::PostTaskAndReply(const Location& from_here,
                                  OnceClosure task,
                                  OnceClosure reply) {
  return ThreadPool::PostTaskAndReply(from_here, { base::MayBlock() }, std::move(task),
                                      std::move(reply));
}
