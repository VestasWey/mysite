...
void TaskQueue::ShutdownTaskQueueGracefully() {
  ...
  DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
  // modify [
  // 传参的 TakeTaskQueueImpl() 内部会采用std::move(impl_);的方式将impl_置空，
  // 最后执行到impl_->sequence_manager()的时候会引发空指针异常。
  // hint：似乎clang编译运行这段代码就没有问题，是因为clang和msvc函数调用执行的顺序不同？？
  /*impl_->SetObserver(nullptr);
  impl_->sequence_manager()->ShutdownTaskQueueGracefully(TakeTaskQueueImpl());*/
  auto raw_ptr = impl_.get();
  raw_ptr->SetObserver(nullptr);
  raw_ptr->sequence_manager()->ShutdownTaskQueueGracefully(TakeTaskQueueImpl());
  // ] modify
}
