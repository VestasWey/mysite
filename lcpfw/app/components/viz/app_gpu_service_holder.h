...
class AppGpuServiceHolder : public gpu::GpuInProcessThreadServiceDelegate {
 public:
  class ScopedResetter {
   public:
    ~ScopedResetter() { AppGpuServiceHolder::ResetInstance(); }
  };
  ...
  static AppGpuServiceHolder* GetInstance();
  ...
  explicit AppGpuServiceHolder(const gpu::GpuPreferences& preferences);
  ~AppGpuServiceHolder() override;
 ...
 private:
  friend struct base::DefaultSingletonTraits<AppGpuServiceHolder>;
  ...
  DISALLOW_COPY_AND_ASSIGN(AppGpuServiceHolder);
};
