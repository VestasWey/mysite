...
#include "app_discardable_memory_allocator.h"
...
AppDiscardableMemoryAllocator::AllocateLockedDiscardableMemory(size_t size) {
  return std::make_unique<DiscardableMemoryImpl>(size);
}

size_t AppDiscardableMemoryAllocator::GetBytesAllocated() const {
  return 0U;
}
