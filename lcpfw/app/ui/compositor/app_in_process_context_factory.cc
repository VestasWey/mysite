...
class DirectOutputSurface : public viz::OutputSurface {
 public:
  explicit DirectOutputSurface(
      scoped_refptr<AppInProcessContextProvider> context_provider)
  ...
  uint32_t GetFramebufferCopyTextureFormat() override {
    auto* gl = static_cast<AppInProcessContextProvider*>(context_provider());
...
class AppInProcessContextFactory::PerCompositorData
...
AppInProcessContextFactory::AppInProcessContextFactory(
    viz::HostFrameSinkManager* host_frame_sink_manager,
    viz::FrameSinkManagerImpl* frame_sink_manager)
    : AppInProcessContextFactory(host_frame_sink_manager,
                              frame_sink_manager,
...
void AppInProcessContextFactory::SetUseFastRefreshRateForTests() {
  refresh_rate_ = 200.0;
}

void AppInProcessContextFactory::CreateLayerTreeFrameSink(
  ...
  if (!shared_worker_context_provider_ || shared_worker_context_provider_lost) {
    ...
    shared_worker_context_provider_ = AppInProcessContextProvider::CreateOffscreen(
  ...
  scoped_refptr<AppInProcessContextProvider> context_provider =
      AppInProcessContextProvider::Create(attribs, &gpu_memory_buffer_manager_,
  ...
  if (renderer_settings_.use_skia_renderer) {
    auto skia_deps = std::make_unique<viz::SkiaOutputSurfaceDependencyImpl>(
        viz::AppGpuServiceHolder::GetInstance()->gpu_service(),
    ...
    display_output_surface = std::make_unique<cc::AppPixelOutputSurface>(
  ...
  auto layer_tree_frame_sink = std::make_unique<AppDirectLayerTreeFrameSink>(
...
AppInProcessContextFactory::SharedMainThreadContextProvider() {
  ...
  constexpr bool support_locking = false;
  shared_main_thread_contexts_ = AppInProcessContextProvider::CreateOffscreen(
      &gpu_memory_buffer_manager_, &image_factory_, support_locking);
...
AppInProcessContextFactory::SharedMainThreadRasterContextProvider() {
...
void AppInProcessContextFactory::RemoveCompositor(Compositor* compositor) {
...
AppInProcessContextFactory::GetGpuMemoryBufferManager() {
  return &gpu_memory_buffer_manager_;
}

cc::TaskGraphRunner* AppInProcessContextFactory::GetTaskGraphRunner() {
  return &task_graph_runner_;
}

viz::FrameSinkId AppInProcessContextFactory::AllocateFrameSinkId() {
  return frame_sink_id_allocator_.NextFrameSinkId();
}

viz::SubtreeCaptureId AppInProcessContextFactory::AllocateSubtreeCaptureId() {
  return subtree_capture_id_allocator_.NextSubtreeCaptureId();
}

viz::HostFrameSinkManager* AppInProcessContextFactory::GetHostFrameSinkManager() {
  return host_frame_sink_manager_;
}

SkMatrix44 AppInProcessContextFactory::GetOutputColorMatrix(
...
gfx::DisplayColorSpaces AppInProcessContextFactory::GetDisplayColorSpaces(
    Compositor* compositor) const {
  auto iter = per_compositor_data_.find(compositor);
  if (iter == per_compositor_data_.end())
...
base::TimeTicks AppInProcessContextFactory::GetDisplayVSyncTimeBase(
    Compositor* compositor) const {
  auto iter = per_compositor_data_.find(compositor);
  if (iter == per_compositor_data_.end())
...
base::TimeDelta AppInProcessContextFactory::GetDisplayVSyncTimeInterval(
    Compositor* compositor) const {
  auto iter = per_compositor_data_.find(compositor);
  if (iter == per_compositor_data_.end())
...
void AppInProcessContextFactory::ResetDisplayOutputParameters(
...
AppInProcessContextFactory::PerCompositorData*
AppInProcessContextFactory::CreatePerCompositorData(Compositor* compositor) {
